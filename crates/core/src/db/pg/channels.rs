...
#[derive(Debug, Clone)]
pub struct PgInstanceChannelStore {
    pool: PgPool,
}

impl PgInstanceChannelStore {
    pub fn new(pool: PgPool) -> Self {
        Self { pool }
    }
}

fn channel_from_row(row: &sqlx::postgres::PgRow) -> Result<InstanceChannelRow, StoreError> {
    Ok(InstanceChannelRow {
        ...
        webhook_secret_name: row
            ...
            .map_err(map_sqlx)?,
        enabled: row.try_get::<bool, _>("enabled").map_err(map_sqlx)?,
        last_inbound_at: row
...
fn delivery_from_row(row: &sqlx::postgres::PgRow) -> Result<ChannelDeliveryRow, StoreError> {
...
#[async_trait]
impl InstanceChannelStore for PgInstanceChannelStore {
    ...
    async fn delete(&self, instance_id: &str, kind: &str) -> Result<(), StoreError> {
        sqlx::query("DELETE FROM instance_channels WHERE instance_id = $1 AND kind = $2")
            .bind(instance_id)
            .bind(kind)
            .execute(&self.pool)
            .await
    ...
    async fn set_enabled(
        ...
        let row = sqlx::query(
            "UPDATE instance_channels
             SET enabled = $1
             WHERE instance_id = $2 AND kind = $3
             RETURNING id, instance_id, kind, handle, secret_name, webhook_secret_name,
                       enabled, last_inbound_at, created_at",
        )
        .bind(enabled)
    ...
    async fn update_last_inbound_at(
        ...
        sqlx::query(
            "UPDATE instance_channels SET last_inbound_at = $1 WHERE instance_id = $2 AND kind = $3",
        )
        .bind(at)
        .bind(instance_id)
        .bind(kind)
