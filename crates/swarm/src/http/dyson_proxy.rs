...
//!   otherwise the `dyson_swarm_session` cookie (an opaque server-side
//!   session id with `Domain=<hostname>`) so plain URL-bar navigation
//!   to a Dyson subdomain — open-in-new-tab, image src, anchor click —
//!   carries credentials. The cookie is `SameSite=Strict` and the
//!   dispatcher additionally enforces `Origin`/`Referer` on non-GET
//!   methods (post-F2 hardening); requests without a matching origin
//!   are rejected before reaching the cube.
