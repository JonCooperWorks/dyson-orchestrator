...
| Workspace identity and memory | `instance_state_files` namesp%ace `workspace` | `/var/lib/dyson/workspace` hot cache | Mirrored durable paths: top-level Markdown, `memory/*.md`, `kb/**`, `skills/**`, and public-channel memory/audit files. |
